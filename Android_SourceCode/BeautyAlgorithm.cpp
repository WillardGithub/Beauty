...
//added at 2020-5-10
double   GlobalBuffing(Mat src, double ratio, Mat& dst)   //整体磨皮
{
	if (src.empty())
	{
		return -1;
...
double  LocalBuffing(Mat src, double ratio, Mat& dst, short* p)//局部(人脸部位)磨皮
{
	if (src.empty() || !p) //判断输入图像及人脸关键点指针是否为空
	{
		return -1;
...
double  GlobalWhitening(Mat src, double ratio, Mat& dst, short* p)//全局美白
{
	if (src.empty() || !p) //判断输入图像及人脸关键点指针是否为空
	{
		return -1;
...
double   Whitening(Mat src, double ratio, Mat& dst, short* p)//局部美白
{
/*#if 0
	if (src.empty() || !p) //判断输入图像及人脸关键点指针是否为空
	{
...
double   FrozenFilter(Mat src, Mat& dst, short* p) //冰冻滤镜
{
	if (src.empty() || !p) //判断输入图像及人脸关键点指针是否为空
	{
		return -1;
